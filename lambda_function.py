...
def process_api_gateway_webhook(event, context):
    ...
    try:
        ...
        # Send to Firehose for Redshift
        try:
            stream_name = os.environ.get('FIREHOSE_STREAM_NAME', 'meraki-redshift-stream')
            
            firehose_client.put_record(
                DeliveryStreamName=stream_name,
                Record={'Data': json.dumps(webhook_data) + '\n'}
            )
        except Exception as firehose_error:
            print(f"Firehose error: {firehose_error}")
            # Log for manual recovery
            print(f"Webhook data for manual recovery: {json.dumps(webhook_data)}")
